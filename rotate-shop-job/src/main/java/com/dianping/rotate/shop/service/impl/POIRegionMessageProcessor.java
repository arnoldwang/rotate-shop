...
public class POIRegionMessageProcessor implements MessageProcessor {
    ...
    public void process(){
        while(Switch.on()){
            try {
                List<MessageEntity>  messages = messageQueueDAO.getMessage(MessageSource.PERSON, POIMessageType.SHOP_REGION,
                ...
                for(final MessageEntity msg:messages){
                    tasks.add(new Callable<Integer>() {
                        ...
                        public Integer call() throws Exception {
                            return messageProcessService.messageProcess(msg,POIMessageType.SHOP_REGION);
                        }
                    });
                }
