 ...
 */
public class POIMergeRecoverMessageProcessor implements MessageProcessor {
    ...
    public void process(){
        while(true){
            List<MessageEntity> messages = messageQueueDAO.getMessage(MessageSource.PERSON, POIMessageType.SHOP_MERGE_RECOVER,PROCESS_MESSAGE_LIMIT);
            Collection<Callable<Integer>> tasks=new ArrayList<Callable<Integer>>();
            for(final MessageEntity msg:messages){
                tasks.add(new Callable<Integer>() {
                    @Override
                    public Integer call() throws Exception {
                        return messageProcessService.messageProcess(msg,POIMessageType.SHOP_MERGE_RECOVER);
                    }
                });
            }
            try{
